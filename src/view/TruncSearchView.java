...
public class TruncSearchView extends JFrame {
    ...
    private final JTextField txtDigitPositions; // Nuevo campo para posiciones de dígitos
    private final JLabel lblResult;

    public TruncSearchView() {
        // Basic window configuration
        setTitle("Función Hash por Extracción de Dígitos");
        setSize(600, 1000); // Aumentamos un poco la altura para el nuevo campo
        ...
        // Panel para definir posiciones de dígitos (NUEVO)
        JPanel digitPositionsPanel = createControlPanel();
        JLabel lblDigitPositions = new JLabel("Posiciones de dígitos (ej: 2,5,9):");
        ...
        txtDigitPositions.setToolTipText("Ingrese las posiciones de los dígitos separadas por comas (ej: 2,5,9)");

        digitPositionsPanel.add(lblDigitPositions);
        digitPositionsPanel.add(txtDigitPositions);

        verticalControlPanel.add(digitPositionsPanel);
        verticalControlPanel.add(Box.createRigidArea(new Dimension(0, 15)));
    ...
    public int[] getSelectedDigitPositions() {
        String positions = txtDigitPositions.getText().trim();
        if (positions.isEmpty()) {
            // Si no se especifica, usamos las posiciones 1 y 2 por defecto
            return new int[]{1, 2};
        }

        // Dividir la cadena por comas
        String[] positionsArray = positions.split(",");
        int[] result = new int[positionsArray.length];

        // Convertir cada posición a entero
        for (int i = 0; i < positionsArray.length; i++) {
            try {
                result[i] = Integer.parseInt(positionsArray[i].trim());
            } catch (NumberFormatException e) {
                // Si hay error, usamos la posición i+1
                result[i] = i + 1;
            }
        }
