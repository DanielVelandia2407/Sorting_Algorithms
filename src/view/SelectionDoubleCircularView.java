...
public class SelectionDoubleCircularView extends JFrame {
    ...
    public SelectionDoubleCircularView() {
        // Basic window configuration
        setTitle("Selection Sort - Lista Doble Circular");
        setSize(900, 950);
        ...
        JLabel lblTitle = new JLabel("Selection Sort - Lista Doble Circular");
        lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 22));
        lblTitle.setForeground(Color.WHITE);
        lblTitle.setAlignmentX(Component.CENTER_ALIGNMENT);

        JLabel lblSubtitle = new JLabel("Navegación bidireccional en estructura circular cerrada");
        ...
        // Create table model with columns for the politicians including next and prev
        tableModel = new DefaultTableModel(new Object[]{"ID", "Nombre", "Dinero a Robar ($)", "Anterior", "Siguiente"}, 0) {
            @Override
        ...
        // Center align ID, Anterior and Siguiente
        ...
        txtNumElements.setText("10"); // Default value
        ...
        btnSort = createStyledButton("Ordenar (Selection Sort)", new Color(41, 128, 185));
    ...
    // Method to highlight multiple rows (for Selection Sort visualization)
    public void highlightRows(int row1, int row2) {
        dataTable.clearSelection();
        if (row1 >= 0 && row1 < dataTable.getRowCount()) {
            dataTable.addRowSelectionInterval(row1, row1);
        }
        if (row2 >= 0 && row2 < dataTable.getRowCount()) {
            dataTable.addRowSelectionInterval(row2, row2);
        }
        if (row1 >= 0 && row1 < dataTable.getRowCount()) {
            dataTable.scrollRectToVisible(dataTable.getCellRect(row1, 0, true));
        }
    }
